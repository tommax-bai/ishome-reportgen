...
BANNED_GROUP_ROUTE = {
    ...
    # 造价域两组（2026-08-31 第一次六章整册真跑后补：此前七个词全挂在 domain_extra 这个桶名下，
    # 查不到路、一律退到兜底那句，模型三次打回都没拿到"为什么不行"）。
    "deal": "这是在替谁把这活的钱定死、把买卖谈成 → 这一章只讲钱花在哪儿、各项占多大头，成交那一步不归你说",
    "unbacked_verdict": "这是替业主下好坏判断，而好坏要有数撑着 → 只摆各项占多少、差在哪，好不好由业主自己看",
}
