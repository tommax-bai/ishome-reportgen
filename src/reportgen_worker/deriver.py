...
DERIVE_CALL_POINT = "report-derive"
"""这一处 AI 判断的名字，随每次调用报给网关（网关按它把调用记到正确的名下）。

网关只看得见模型名，分不出"定这一章讲什么"与"写报告卡片"——两步今天恰好同底模，同一个模型名
底下混着三处判断。名字的真源是判官台那张基本信息表，此处照抄不改；与逻辑模型名同域不同物：
模型名说的是"调哪个模型"、换模型就变，这个说的是这一步本身。

三个类各写一份而不抽公共件：import-linter 锁死推导/写作/判官互不可见（判官与写手共享任何
东西都是同源漂移的入口），这一行的重复是那条分界的代价，不是疏忽。"""
...
class DeriveRequest(BaseModel):
    ...
    """这次运行的编号，只随调用报给网关做调用记录，**不进 prompt、不参与推导**。

    整册一个编号、六章各自成章、每章还有重写轮，不带编号网关那边就拼不回"这几十次调用是同一册"。
    取现成的：整册的运行标识（Temporal workflow id）+ 章名 + 第几轮，见 `activities`。
    取不到就是 None——不编一个看着像编号的东西。
...
def build_derive_messages(request: DeriveRequest) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "你这一步连值都没拿到；\n"
        "3. **先把落点按场景归组，再给每组写一条主张**——"
        "一个落点一条主张等于把落点表换了个排版，那正是要修的东西。"
        "场景＝同一个空间里的同一件事（同一件家具、同一个动作）："
        "床面高和床侧净距同属「床」，**就该进同一条主张**，正文里各给各的理由；"
        "厨房的两排间距、水槽深、吊柜底沿、冰箱散热同属「在厨房做饭」，归一条；"
        "主通道和次通道同属「在家里走动」，归一条。"
        "主张是这一组的**一句判断**，不是这组参数的清单——写作步会把它当卡片标题；"
        "你这一步照旧一个数都不写（数是写作步从落点取的，见第 2 条）；\n"
        "3a. 归组的好例与坏例（只看形态，句子别抄）：好例＝「厨房」一条，挂两排间距、水槽深、"
        "吊柜底沿、冰箱散热四条落点，主张说的是这家人在厨房做饭该照着谁的身体定；"
        "坏例＝把马桶、玄关柜、衣柜、沙发、走廊这几样互不相干的东西塞进同一条，"
        "主张只能写成几个参数排成一排——那是清单不是判断，写作步只好把一串参数排进标题；\n"
        "3b. **不许把同组落点写成相互约束的关系**（说 A 的取值牵制 B、两者要配合着调整之类）——"
        "数据里没有这种联动就不许说。归组照做，关系别编——这是两件事；"
        "每条主张挂上它这一组的落点 id（可以挂多条；说取舍不说数的主张也可以一条不挂）；"
        "落点是这一域**已经算出来的**东西，能归进某个场景的就别丢在外面，"
        "但也不许为了塞下所有落点把不相干的场景硬凑成一条；\n"
        ...
        "（条目后面括号里那句就是理由）——「因为你家〈括号里那条理由〉，所以…」这种话才是业主要看的，"
        "凭空说「这里要留个位」不是；\n"
        ...
        "3f. 清单里标着「合计」的条目是求值线按这家的量算出来的钱——**必须落进某条主张**"
        "（它是业主最想知道的那句「大概要花多少」，不许因为取舍把它留在外面）；\n"
        "3e. **关于这家人的事实只有两处来源**：下面「这套户型」那一行，和户型条目括号里的理由。"
        "这两处没写的家庭事实——房龄、家庭成员、生活习惯、哪间房归谁用、哪儿有什么设备——"
        "一个都不许出现，也不许从条目内容里引申出来；画像里给了什么就只能用什么；\n"
        "4. 能下结论的题目只有这些："
        + backed
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            response = await client.post(
                ...
                json={
                    ...
                    "temperature": 0,
                    # 这次调用记在谁名下（网关的调用记录按这两样归口，见 DERIVE_CALL_POINT）。
                    # LiteLLM 自己消费 metadata，不透传给厂商。
                    "metadata": {
                        "call_point": DERIVE_CALL_POINT,
