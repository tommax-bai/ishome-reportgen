...
def build_messages(request: WriterRequest) -> list[dict[str, str]]:
    ...
    claims_discipline = (
        ...
        "**同一条落点的记号只在一张卡里出现一次**（哪张卡讲它，数就在哪张卡给；"
        "别的卡要提到它，用人话带过，不再把记号句抄一遍）；"
    ...
    if gap_lines:
        user_parts.append(
            # 这一块 2026-08-31 两次改判，记住第二次的理由。
            # 一改：原文写着"可坦白留待现场确认"，真跑成册里四章都把它写成了"等你确认沙发落位"
            #   「等你提供物品清单」——**把我们自己没做完的活说成业主没交作业**。求值线的缺口 reason
            #   只有三种（missing_input / formula_not_implemented / empty_definition），没有一种的
            #   意思是"等客户告诉我们"。于是改成"我们下一步算给他"。
            # 二改（用户当场否掉一改）：**那个"以后"本身就不该存在**——原话「不要在报告里写我们后面
            #   算给他，这报告都给客户了，我们后面怎么算给他？」「我们不应该有后面算给他这个概念」。
            #   报告是一次性交付物：交付那一刻要么有这个数，要么这件事根本不进报告。把锅从业主挪到
            #   我们身上，只是换了个人背，承诺照样没有送达渠道。现口径＝**没有值就不写**（规则 4.18
            #   宁薄勿撑），缺口只作为"别编它"的禁令下发，不作为可写的题材。
